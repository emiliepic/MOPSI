...
*
* Author: Renaud Marlet (c) 2011-2017
***************************************************************************/
...
int main() {
    ...
    ql->value()++;                                       // qn=[_,_,2,4]
    // Node modifications
    qn->child(NW)=new QuadLeaf<int>(6);                  // qn=[6,_,2,3]
    qn->child(NW)=new QuadNode<int>(0,0,0,qn->child(NW));// qn=[[_,_,_,6],_,2,3]
    qn->child(NE)=new QuadLeaf<int>(7);                  // qn=[[_,_,_,6],7,2,4]
    qn->child(SE)=new QuadNode<int>(0,0,qn->child(SE),0);
    // qn=[[_,_,_,6],7,[_,_,2,_],4]
    // Modification with (sub)typing test: increment leaves at depth 1
    for(int d=0; d<nQuadDir; d++) {
        QuadTree<int>* q=qn->child(d);
        if (q && q->isLeaf()) q->value()++;    // qn = [[_,_,_,6],8,[_,_,2,_],5]
    }
    qn->child(NW) = new QuadNode<int>(0,0,qn->child(NW),0);
    ...
    */
    ...
    try {
        ...
        std::cout << "Test d'erreur: " << e.what() << std::endl;
